...
func (b *BidBoard) SavePayloadUtilsOpenBid(slot uint64, blockhash string, payloadUtils *utils.GetPayloadResponse) error {

	/*
		It is used to keep the Execution Payload which is used in get payload

			Bid Key-
			bidKeyBuilderUtils-open-slot
								|--blockhash =
								|--blockhash =
								|--blockhash =
	*/

	bidKey := fmt.Sprintf("%s-%d", bidKeyOpenBuilderUtils, slot)
	err := b.redisInterface.HSetObj(bidKey, blockhash, payloadUtils, b.bidTimeout)
	if err != nil {
		return err
	}
...
func (b *BidBoard) OpenPayloadUtils(slot uint64, blockhash string) (utils.GetPayloadResponse, error) {

	bidKey := fmt.Sprintf("%s-%d", bidKeyBuilderUtils, slot)
	utilsRelay := new(utils.GetPayloadResponse)
	value, err := b.redisInterface.Client.HGet(context.Background(), bidKey, blockhash).Result()
	if err != nil {
		return utils.GetPayloadResponse{}, err
	}
	err = json.Unmarshal([]byte(value), &utilsRelay)
	return *utilsRelay, err
}
