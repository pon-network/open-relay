...
func NewDatabase(url string,
	...
	if openRelay {
		...
		migrationFilePath := filepath.Join(currentDir, "database", "migrations", "0001_initialize_tablesOpenRelay.up.sql")
...
func (database *DatabaseInterface) PutBuilderOpenBlockSubmission(ctx context.Context,
	builderSubmission databaseTypes.BuilderBlockDatabase) error {
	query := `INSERT INTO builder_open_block_submissions
		(slot, builder_pubkey, bid_value, builder_signature) VALUES
		($1, $2, $3, $4)`
	_, err := database.DB.ExecContext(
		ctx,
		query,
