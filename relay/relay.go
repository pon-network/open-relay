...
import (
	...
	"github.com/pon-network/open-relay/bids"
	...
	reporterServer "github.com/pon-network/open-relay/reporter"
	"github.com/pon-network/open-relay/rpbs"
	"github.com/pon-network/open-relay/signing"
	relayUtils "github.com/pon-network/open-relay/utils"
)

func NewRelayAPI(params *RelayParams, log logrus.Entry) (relay *Relay, err error) {
	dataBase, err := database.NewDatabase(params.DbURL, params.DatabaseParams, params.DbDriver, params.DeleteTables, params.OpenRelay)
	if err != nil {
...
func (relay *Relay) Routes() http.Handler {
	...
	if relay.openRelay {
		fmt.Println("Open Relay")
		r.HandleFunc("/relay/v1/builder/validators", relay.handleBuilderGetValidators).Methods(http.MethodGet)
		r.HandleFunc("/relay/v1/builder/blocks", relay.handleSubmitOpenBlock).Methods(http.MethodPost)
	} else {
...
func (relay *Relay) handleBuilderGetValidators(w http.ResponseWriter, req *http.Request) {

	proposerDuties := make([]BuilderGetValidatorsResponseEntry, 0)
	relay.beaconClient.BeaconData.Mu.Lock()
	defer relay.beaconClient.BeaconData.Mu.Unlock()
	epoch := relay.beaconClient.BeaconData.CurrentEpoch

	resp, err := relay.beaconClient.GetSlotProposerMap(epoch)
	if err != nil {
		relay.log.WithError(err).Warn("failed to get slot proposer map")
		relay.RespondError(w, http.StatusInternalServerError, "failed to get slot proposer map")
		return
	}

	for slot, duty := range resp {

		var publicKeyBytes [48]byte
		copy(publicKeyBytes[:], duty.PubkeyHex[:])

		proposerDuties = append(proposerDuties, BuilderGetValidatorsResponseEntry{
			Slot:           slot,
			ValidatorIndex: duty.Index,
			Entry: &boostTypes.SignedValidatorRegistration{
				Signature: *new(boostTypes.Signature),
				Message: &boostTypes.RegisterValidatorRequestMessage{
					Pubkey:       boostTypes.PublicKey(publicKeyBytes),
	...
	resp, err = relay.beaconClient.GetSlotProposerMap(epoch + 1)
	if err != nil {
		relay.log.WithError(err).Warn("failed to get slot proposer map")
		relay.RespondError(w, http.StatusInternalServerError, "failed to get slot proposer map")
		return
	...
	relay.log.Infof("proposer duties updated: %s", strings.Join(_duties, ", "))

	relay.RespondOK(w, proposerDuties)
}
...
func (relay *Relay) handleSubmitOpenBlock(w http.ResponseWriter, req *http.Request) {
	blockTimestamp := time.Now()
	builderBlock := new(BuilderOpenBid)

	if builderBlock.Message() == nil || !builderBlock.HasExecutionPayload() {
		relay.RespondError(w, http.StatusBadRequest, "missing parts of the payload")
		return
	}

	if builderBlock.Value().Cmp(ZeroU256.BigInt()) == 0 || builderBlock.NumTx() == 0 {
		relay.log.Info("submitNewBlock failed: block with 0 value or no txs")
		w.WriteHeader(http.StatusOK)
		return
	}

	/* --------------- check if bid already delivered to proposer --------------- */
	deliveredPayloadBuilder, err := relay.bidBoard.GetPayloadDelivered(builderBlock.Capella.Message.Slot)
	if err != nil && !errors.Is(err, redis.Nil) {
		relay.log.WithError(err).Error("failed to get delivered payload slot from redis")
		relay.RespondError(w, http.StatusBadRequest, "failed to get delivered payload slot from redis")
		return
	} else if err != nil && errors.Is(err, redis.Nil) {
		relay.log.Info("No Payload Sent For Slot") // @dev This is not an error
	} else {
		relay.log.Errorf("Payload Delivered For Slot %d, You Are Late.....", builderBlock.Capella.Message.Slot)
		relay.RespondError(w, http.StatusBadRequest, fmt.Sprintf("Payload For Slot %d Delivered For Builder %s", builderBlock.Capella.Message.Slot, deliveredPayloadBuilder))
		return
	}

	/* -------------------------- verify bls signature -------------------------- */
	// signature := builderBlock.Signature()
	// builderPubkey := builderBlock.BuilderPubkey()
	// ok, err := boostTypes.VerifySignature(builderBlock.Message(), relay.network.DomainBuilder.ToFlashbots(), builderPubkey[:], signature[:]) // @dev Verify Signature
	// if err != nil {
	// 	relay.log.WithError(err).Warn("failed verifying builder signature")
	// 	relay.RespondError(w, http.StatusBadRequest, "failed verifying builder signature")
	// 	return
	// } else if !ok {
	// 	relay.log.Warn("invalid builder signature")
	// 	relay.RespondError(w, http.StatusBadRequest, "invalid signature")
	// 	return
	// }

	/* -------------------------- checking slot number -------------------------- */
	relay.beaconClient.BeaconData.Mu.Lock()
	relaySlot := relay.beaconClient.BeaconData.CurrentSlot
	relay.beaconClient.BeaconData.Mu.Unlock()
	if builderBlock.Capella.Message.Slot != relaySlot && builderBlock.Capella.Message.Slot != relaySlot+1 {
		relay.log.Warnf("submitBlock failed: submission for wrong slot, Expected %d or %d, Got %d", relaySlot, relaySlot+1, builderBlock.Capella.Message.Slot)
		relay.RespondError(w, http.StatusBadRequest, fmt.Sprintf("submission for wrong slot, Expected %d or %d, Got %d", relaySlot, relaySlot+1, builderBlock.Slot()))
		return
	}

	builderDbBlock := databaseTypes.BuilderBlockDatabase{
		Slot:          builderBlock.Slot(),
		BuilderPubkey: builderBlock.BuilderPubkey().String(),
		// BuilderBidString: builderBlock.Capella.String(), // @dev fix in types
		BuilderSignature: builderBlock.Signature().String(),
		BidValue:         *builderBlock.Capella.Message.Value.ToBig(),
	}

	defer func() {
		err := relay.db.PutBuilderOpenBlockSubmission(req.Context(), builderDbBlock)
		if err != nil {
			relay.log.WithError(err).WithField("payload", builderDbBlock).Error("saving builder open block submission to database failed")
			return
		}
	}()

	// get last bid for the builder for the slot
	lastBid, err := relay.bidBoard.BuilderBlockLast(builderBlock.Slot(), builderBlock.BuilderPubkey().String())
	if err != nil {
		if err != redis.Nil {
			relay.log.WithError(err).Error("failed getting latest payload receivedAt from redis")
			relay.RespondError(w, http.StatusInternalServerError, "failed getting latest payload receivedAt from redis")
		...
		relay.RespondError(w, http.StatusBadRequest, fmt.Sprintf("Using newer bid for Builder %s", builderBlock.Capella.Message.BuilderPubkey.String()))
		return
	}

	// convert builderBlock.Capella.ExecutionPayload to *relayTypes.SignedBuilderBlockBid
	executionPayloadHeader, err := builderBlock.ExecutionPayloadHeader()
	if err != nil {
		relay.log.WithError(err).Error("failed getting execution payload header")
		relay.RespondError(w, http.StatusBadRequest, "failed getting execution payload header")
		return
	}

	signedBuilderBid, err := SignedBuilderBidOpen(executionPayloadHeader, builderBlock.Value(), relay.blsSk, relay.publicKey, relay.network.DomainBuilder)
	if err != nil {
		relay.log.WithError(err).Error("could not sign builder bid")
		relay.RespondError(w, http.StatusInternalServerError, err.Error())
		return
	...
	getHeaderResponse, err := builderBlock.GetHeaderResponse(signedBuilderBid)
	if err != nil {
		relay.log.WithError(err).Error("could not get header response")
		relay.RespondError(w, http.StatusInternalServerError, err.Error())
		return
	}

	getPayloadHeaderResponse, err := builderBlock.ExecutionPayloadResponse()
	if err != nil {
		relay.log.WithError(err).Error("failed saving execution payload in redis")
		relay.RespondError(w, http.StatusInternalServerError, err.Error())
		return
	...
	err = relay.bidBoard.SavePayloadUtilsOpenBid(builderBlock.Slot(), builderBlock.Message().BlockHash.String(), getPayloadHeaderResponse)
	if err != nil {
		relay.log.WithError(err).Error("failed saving execution payload in redis")
		relay.RespondError(w, http.StatusInternalServerError, err.Error())
		return
	...
	err = relay.bidBoard.SaveBuilderBid(
		builderBlock.Slot(),
		builderBlock.Message().BlockHash.String(),
		builderBlock.Message().ProposerPubkey.String(),
	...
	highestBidBuilder, highestBidValue, err := relay.bidBoard.AuctionBid(builderBlock.Slot())
	if err != nil {
		relay.log.WithError(err).Error("could not compute top bid")
		relay.RespondError(w, http.StatusInternalServerError, err.Error())
		return
	...
	relay.log.WithFields(logrus.Fields{
		"Builder": builderBlock.BuilderPubkey().String(),
		"Value":   builderBlock.Value(),
	}).Info("received block from builder open relay")
...
func (relay *Relay) handleProposerPayload(mevBoost http.ResponseWriter, req *http.Request) {
	...
	if relay.openRelay {
		...
		blockSubmission, err := relay.bidBoard.OpenPayloadUtils(uint64(slot), blockHash)
		if err != nil {
		...
		defer func() {
			errs := relay.relayutils.SendDiscord(slot, "Open Relay", proposerPubkey)
			if errs != nil {
				relay.log.WithError(errs).Error("Couldn't Send To Discord")
			}
		}()

		relay.RespondOK(mevBoost, &blockSubmission)
